...
public class Programming1 {
	public static String solution(String new_id) {
		...
		for(int i = 0; i < new_id.length(); i++) {
			...
			if((charAt >= 97 && charAt <= 122) || (charAt >= 48 && charAt <= 57) || charAt == 45
					|| charAt == 95 || charAt == 46) {
				answer += charAt;
			}
		}
		...
		// 6단계
		// 길이가 16보다 크면 15까지 자름. 마지막이 .으로 끝난다면 .을 제거
		if(answer.length() >= 16) {
		...
		// 7단계
		// 길이가 2자 이하라면 길이가 3이 될때까지 제일 마지막 문자를 반복해서 끝에 붙임.
		if(answer.length() <= 2) {
	...
	public static void main(String[] args) {
		...
		String new_id1 = "z-+.^.";		// z--
		String new_id2 = "=.=";			// aaa
		String new_id3 = "123_.def";	// 123_.def
		String new_id4 = "abcdefghijklmn.p";		// abcdefghijklmn
